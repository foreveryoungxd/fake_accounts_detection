...
def main():
    ...
    mode = st.sidebar.radio(
        "Режим:",
        options=[
            "Выявление фиктивных аккаунтов в комментариях публикации в Telegram",
            "Выявление фиктивных аккаунтов в Excel-таблице",
            "Аналитика по базе данных фиктивных аккаунтов",
        ],
    )
    ...
    if mode == "Выявление фиктивных аккаунтов в Excel-таблице":
        st.header("Загрузите файл с комментариями")
        st.info("Файл должен иметь расширение XLSX, XLS или CSV")
        with st.expander("📋 Требования к структуре файла", expanded=True):
            st.markdown(
                """
               **Файл должен содержать следующие обязательные колонки:**
               - `date` - дата и время комментария
               - `message_id` - ID сообщения
               - `reply_to_msg_id` - ID сообщения, на которое дан ответ
               - `channel_name` - название Telegram-канала
               - `channel_id` - ID Telegram-канала
               - `sender` - имя аккаунта (может быть пустым)
               - `sender_id` - ID аккаунта
               - `message` - текст сообщения
               - `reactions` - реакции на сообщение
               - `channel_members_count` - количество подписчиков Telegram-канала
               """
            )
        st.image(
            "docs/фото_эталонной_таблицы.png", caption="Пример структуры файла"
        )
        ...
        if uploaded_file is not None:
            try:
                ...
                if len(processed_df) == 0:
                    ...
                    resolution_param = (
                        "H" if time_resolution == "По часам" else "D"
                    )
    ...
    if mode == "Аналитика по базе данных фиктивных аккаунтов":
        st.header("Аналитика по базе данных фиктивных аккаунтов")
        ...
        if st.button("Загрузить данные"):
            ...
            st.info(
                f"Выявлено {len(channel_data)} комментариев, оставленных фиктивными аккаунтами"
            )
            st.dataframe(
                channel_data.rename(
                    columns={
